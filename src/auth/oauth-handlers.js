...
/**
 * 导入 Kiro 账号管理器数据 (完整 JSON)
 * @param {Object} data - Kiro 账号管理器导出的 JSON 数据
 * @returns {Promise<Object>} 导入结果
 */
export async function importKiroManagerData(data) {
    ...
    const timestamp = Date.now();
    // 使用 configs/kiro 作为基础目录，不创建带时间戳的子目录，方便管理
    // 或者为了避免冲突，保持原有的子目录结构
    // 这里为了整洁，我们把所有导入的凭证放在 configs/kiro/imported_accounts 下，或者直接在 configs/kiro 下用文件名区分
    // convert-manager-data.js 使用的是 kiro-{email}-{authType}.json，这里我们沿用这个命名规范

    // 确保目录存在
    const targetDir = path.join(process.cwd(), 'configs', 'kiro');
    await fs.promises.mkdir(targetDir, { recursive: true });

    // 读取现有的 provider_pools.json 以便进行去重和更新
    const providerPoolsFile = CONFIG.PROVIDER_POOLS_FILE_PATH || path.join(process.cwd(), 'configs', 'provider_pools.json');
    let providerPools = {};
    try {
        if (fs.existsSync(providerPoolsFile)) {
            const content = await fs.promises.readFile(providerPoolsFile, 'utf8');
            providerPools = JSON.parse(content);
        }
    } catch (e) {
        console.warn(`${KIRO_OAUTH_CONFIG.logPrefix} Failed to read provider_pools.json:`, e.message);
    }
    ...
    for (const account of data.accounts) {
        try {
            if (!account.credentials || !account.credentials.refreshToken) {
                results.skipped++;
                results.details.push({ email: account.email, success: false, msg: 'Missing credentials' });
                continue;
            }
            ...
            if (creds.authMethod === 'social') {
                credData.authMethod = 'social';
                // Social 模式不需要 clientId/Secret
            } else {
                credData.authMethod = creds.authMethod || 'IdC';
                credData.clientId = creds.clientId;
                credData.clientSecret = creds.clientSecret;
            }

            if (creds.expiresAt) {
                credData.expiresAt = (typeof creds.expiresAt === 'number')
                    ? new Date(creds.expiresAt).toISOString()
                    : creds.expiresAt;
            }

            // 生成文件名
            const emailPrefix = sanitizeFileName(email);
            const authType = creds.authMethod === 'social' ? (creds.provider || 'social').toLowerCase() : 'idc';
            const fileName = `kiro-${emailPrefix}-${authType}.json`;
            const filePath = path.join(targetDir, fileName);

            // 写入文件
            await fs.promises.writeFile(filePath, JSON.stringify(credData, null, 2), 'utf8');
            const relativePath = path.relative(process.cwd(), filePath);

            // 准备 Pool 配置
            const customName = `${account.nickname || email} (${creds.provider || creds.authMethod})`;

            // 检查是否已存在（通过 customName 匹配，或者文件路径匹配）
            // 这里的策略是：优先查找 customName 相同的，更新其路径；
            // 否则查找路径相同的，更新其 customName (不太可能发生，因为路径是我们新生成的);
            // 都不存在则新增。

            // 为了更稳健，我们先找是否有即有的项使用了这个 email 生成的路径（即使 customName 不同）
            // 注意：因为路径包含 email，所以它是唯一的
            let existingEntry = existingPool.find(p => {
                const pPath = p.KIRO_OAUTH_CREDS_FILE_PATH;
                return pPath && (
                    pPath === relativePath ||
                    pPath === `./${relativePath}` ||
                    path.normalize(pPath) === path.normalize(relativePath) ||
                    path.basename(pPath) === fileName
                );
            });

            if (!existingEntry) {
                // 如果没有通过路径找到，尝试通过 customName 找
                existingEntry = existingPool.find(p => p.customName === customName);
            }

            if (existingEntry) {
                // 更新现有项
                existingEntry.KIRO_OAUTH_CREDS_FILE_PATH = `./${formatSystemPath(relativePath).replace(/^[\\/]/, '')}`; // 确保是相对路径格式
                // 保持 uuid 和统计信息不变
                console.log(`${KIRO_OAUTH_CONFIG.logPrefix} Updated existing pool entry for: ${customName}`);
            } else {
                // 新增项
                const newEntry = {
                    customName: customName,
                    KIRO_OAUTH_CREDS_FILE_PATH: `./${formatSystemPath(relativePath).replace(/^[\\/]/, '')}`,
                ...
                existingPool.push(newEntry);
                console.log(`${KIRO_OAUTH_CONFIG.logPrefix} Added new pool entry for: ${customName}`);
            }

            results.success++;
            results.details.push({ email: email, success: true, path: relativePath });

            // 广播单个成功事件（可选，避免太多通知刷屏，可以只最后广播一次）
        } catch (err) {
            console.error(`${KIRO_OAUTH_CONFIG.logPrefix} Error importing account ${account ? account.email : 'unknown'}:`, err);
            results.skipped++;
            results.details.push({ email: account ? account.email : 'unknown', success: false, msg: err.message });
        }
    }

    // 保存 provider_pools.json
    try {
        await fs.promises.writeFile(providerPoolsFile, JSON.stringify(providerPools, null, 2), 'utf8');
        console.log(`${KIRO_OAUTH_CONFIG.logPrefix} provider_pools.json updated successfully.`);
    } catch (e) {
        console.error(`${KIRO_OAUTH_CONFIG.logPrefix} Failed to save provider_pools.json:`, e.message);
        throw new Error(`Failed to update provider pools: ${e.message}`);
    }

    // 刷新内存中的配置（通过 autoLinkProviderConfigs 或类似机制，但这里主要依靠重启或重载配置）
    // 调用 autoLinkProviderConfigs 确保一切同步，尽管我们已经手动写入了
    try {
        await autoLinkProviderConfigs(CONFIG);
    } catch (e) {
        console.warn(`${KIRO_OAUTH_CONFIG.logPrefix} Auto-link failed (non-fatal):`, e.message);
    }

    return results;
}

// 辅助函数：简单的路径格式化，确保统一使用正斜杠（如果是 Windows 环境可能需要注意，但 JSON 中通常用 / 或 \\ 均可，Node.js path 模块会自动处理）
function formatSystemPath(p) {
    return p.replace(/\\/g, '/');
}

...
/**
 * 导入 Kiro 账号管理器完整数据 (Batch Import from Manager Data)
 */
export async function handleImportKiroManagerData(req, res) {
    try {
        const body = await getRequestBody(req);
        ...
        if (!body || !body.accounts || !Array.isArray(body.accounts)) {
            ...
            res.end(JSON.stringify({
                success: false,
                error: 'Invalid format. Expected JSON with "accounts" array.'
            }));
            return true;
        }

        console.log(`[Kiro Import] Starting import of ${body.accounts.length} accounts from manager data...`);

        const result = await importKiroManagerData(body);

        console.log(`[Kiro Import] Completed: ${result.success} success, ${result.skipped} skipped`);
        ...
        res.end(JSON.stringify({
            ...
            details: result.details,
            message: `Successfully imported ${result.success} accounts.`
        }));
        return true;

    } catch (error) {
        console.error('[Kiro Import] Error:', error);
        res.writeHead(500, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({
            success: false,
            error: error.message

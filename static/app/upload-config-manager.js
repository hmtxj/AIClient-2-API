...
function initUploadConfigManager() {
    ...
    // Kiro 账号导入按钮
    const kiroImportBtn = document.getElementById('btnImportKiroData');
    if (kiroImportBtn) {
        kiroImportBtn.addEventListener('click', handleKiroImport);
    }
 ...
 * 处理 Kiro 账号导入
 */
async function handleKiroImport() {
    const fileInput = document.getElementById('kiroImportFile');
    const resultDiv = document.getElementById('kiroImportResult');
    const importBtn = document.getElementById('btnImportKiroData');
    ...
    if (!fileInput || !fileInput.files || fileInput.files.length === 0) {
        showToast(t('common.error'), '请选择 Kiro 账号备份文件 (.json)', 'error');
        return;
    }
    ...
    if (!file.name.toLowerCase().endsWith('.json')) {
        showToast(t('common.error'), '文件格式错误，仅支持 .json 文件', 'error');
        return;
    }
        ...
        try {
            ...
            try {
                ...
                if (!data.accounts || !Array.isArray(data.accounts)) {
                    throw new Error('JSON 格式无效：未找到 "accounts" 数组');
                }
            } catch (err) {
                showToast(t('common.error'), '文件解析失败: ' + err.message, 'error');
                if (importBtn) {
            ...
            if (result.success) {
                ...
                if (resultDiv) {
                    ...
                    resultDiv.style.backgroundColor = 'rgba(40, 167, 69, 0.1)';
                    resultDiv.style.color = '#28a745';
                    resultDiv.innerHTML = `
                        <strong><i class="fas fa-check-circle"></i> 导入成功!</strong><br>
                        总计: ${result.total} | 成功: ${result.successCount} | 跳过: ${result.skippedCount}<br>
                        <ul style="margin-top: 5px; padding-left: 20px; max-height: 100px; overflow-y: auto;">
                            ${result.details.filter(d => d.success).map(d => `<li>${d.email} -> ${d.path}</li>`).join('')}
                        </ul>
                    `;
                }
                ...
                showToast(t('common.error'), result.error || '导入失败', 'error');
                if (resultDiv) {
                    resultDiv.style.display = 'block';
                    resultDiv.className = 'alert alert-danger';
                    resultDiv.style.backgroundColor = 'rgba(220, 53, 69, 0.1)';
                    resultDiv.style.color = '#dc3545';
                    resultDiv.innerText = '导入失败: ' + (result.error || '未知错误');
                }
            }

        } catch (error) {
            console.error('导入出错:', error);
            showToast(t('common.error'), '导入请求失败: ' + error.message, 'error');
        } finally {
            // 恢复按钮状态
            if (importBtn) {
                importBtn.disabled = false;
    ...
    reader.onerror = () => {
        showToast(t('common.error'), '文件读取失败', 'error');
        if (importBtn) {
            importBtn.disabled = false;
            importBtn.innerHTML = '<i class="fas fa-upload"></i> 开始导入';
        }
